...
export class ReceivingDetailComponent implements OnInit {
  ...
  async getData() {
    try {
      const req = await this._globalService.runRequest(
        'GET',
        `Api/v1/receiving/${this.schema.TransId}`,
        [],
      );
  ...
  async addUserAnswer(index,Questions,value,Tipe="") {
    ...
    if(Tipe=='Checkboxes'){
      console.log(this.schema.Questions[index]['AnswerValue'],'fisrst');
      if(!this.schema.Questions[index]['AnswerValue']){
        this.schema.Questions[index]['AnswerValue']=[];
        this.schema.Questions[index]['AnswerValue'].push(value);
      }
      else if(this.schema.Questions[index]['AnswerValue'].includes(value)){
        this.schema.Questions[index]['AnswerValue']= this.removeItem(this.schema.Questions[index]['AnswerValue'],value);
      }else{
        this.schema.Questions[index]['AnswerValue'].push(value);
      }
    ...
    console.log(this.schema.Questions[index]['AnswerValue'],'answervalue');
