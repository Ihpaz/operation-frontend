...
@Component({
  selector: 'app-soup-quality',
  templateUrl: './soup-quality.component.html',
  styleUrls: ['./soup-quality.component.css']
})
export class SoupQualityComponent implements OnInit {
  ...
  tblSchema: any =  {
    ...
    requestOptions: {
      path: "Api/v1/soup-quality/datatable",
      params: []
    },
  ...
  onFilter() {
    this._globalService.eventPublish('loadTableSAM:MsSoupQuality', true);
  }

  goToDetail(){
    this._globalService.redirectPage(`soup-quality/detail`, [], true);
  }

  goToDetailWithId(id:number){
    this._globalService.redirectPage(`soup-quality/detail`, [{key:'id',value:id}], true);
  }
    ...
    if (alertData.isYes) {
      try {
        ...
        await this._globalService.runRequest(
          'DELETE',
          `Api/v1/soup-quality/remove/${action.id}`,
          [],
          
        );

        this._globalService.showNotif('Success delete soup quality!');
        await this._globalService.eventPublish('loadTable:MsSoupQuality', true);
