...
@Component({
  selector: 'app-kitchen-prep-dashboard',
  templateUrl: './kitchen-prep-dashboard.component.html',
  styleUrls: ['./kitchen-prep-dashboard.component.scss']
})
export class KitchenPrepDashboardComponent implements OnInit {
  @ViewChild(BaseChartDirective) chart: BaseChartDirective | undefined;
  @ViewChild('chartCanvas') chartCanvas: ElementRef;
  
 
  tblSchema2: any =  {
    name: "MsKitchenCompDashboard",
    pageSize: [
      10,
      50,
      100
    ...
    requestOptions: {
      path: "Api/v1/kitchen-prep/get-dashboard-kitchenprep-comp/datatable",
      pathDownload: "Api/v1/kitchen-prep/download-excel-comp",
      params: [ 
      { key: 'Year', value: null },
 ...
 year:number[]=[];
  ...
  filter(){
    // this.getDashboardSAM()
     this._globalService.eventPublish('loadTable:MsKitchenCompDashboard', true);
  }
  ...
  async getDashboardKitchenPrep() {
    try {
      // this._dynamicService.showLoader();
      let url='Api/v1/kitchen-prep/dashboard-kitchenprepbar-comp-ByMonth';
      const req= await this._globalService.runRequest(
         'POST',
          url,
         [],
  ...
  filterGrafik(){
   ...
   async getData() {
    try {
      ...
      const req = await this._globalService.runRequest(
        'POST',
        `api/v1/kitchen-prep/importRawData`,
